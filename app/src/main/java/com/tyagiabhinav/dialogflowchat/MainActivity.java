...
import com.tyagiabhinav.dialogflowchat.utility.MathCalculate;
...
//        String result = Tools.benWordToNumber("৬০০ বিয়োগ ৩০০");
//        Log.d(TAG, "onCreate: MathResult "+result);
...
//    }
    ...
    private void initV2Chatbot() {
        try {
            InputStream stream = getResources().openRawResource(R.raw.agent_bangla);
    ...
    private void sendMessage(View view) {
        ...
        if (msg.trim().isEmpty()) {
            ...
            QueryInput queryInput = QueryInput.newBuilder().setText(TextInput.newBuilder().setText(msg).setLanguageCode("bn-BD")).build();
    ...
    public void callbackV2(DetectIntentResponse response) {
        if (response != null) {
            ...
            String query_text = response.getQueryResult().getQueryText();
            ...
            try {
                intentAction(intent_action, query_text);
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    private void intentAction(String intent_action, String query_text) {
        if (intent_action == null || TextUtils.isEmpty(intent_action)) {
            return;
        }

        Log.d(TAG, "intentAction: " + intent_action);
        ...
        switch (intent_action) {
            ...
            case "please_dance":
                ...
                break;
            case "arithmatic_operations":

                String result = Tools.benWordToNumber(query_text);

                texttoSpeak(result);
                showTextView(result, BOT);
                break;
...
//                showTextView(template, BOT);
    ...
    public void onResults(Bundle bundle) {
        ...
        QueryInput queryInput = QueryInput.newBuilder().setText(TextInput.newBuilder().setText(data.get(0)).setLanguageCode("bn-BD")).build();
        new RequestJavaV2Task(MainActivity.this, session, sessionsClient, queryInput).execute();
